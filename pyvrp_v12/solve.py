...
import pyvrp_v12.search
from pyvrp_v12.GeneticAlgorithm import GeneticAlgorithm, GeneticAlgorithmParams
from pyvrp_v12.PenaltyManager import PenaltyManager, PenaltyParams
from pyvrp_v12.Population import Population, PopulationParams
from pyvrp_v12._pyvrp import ProblemData, RandomNumberGenerator, Solution
...
if TYPE_CHECKING:
    ...
    from pyvrp_v12.stop import StoppingCriterion
...
class SolveParams:
    ...
    def from_file(cls, loc: str | pathlib.Path):
        ...
        if "node_ops" in data:
            node_ops = [getattr(pyvrp_v12.search, op) for op in data["node_ops"]]

        route_ops = ROUTE_OPERATORS
        if "route_ops" in data:
            route_ops = [getattr(pyvrp_v12.search, op) for op in data["route_ops"]]
